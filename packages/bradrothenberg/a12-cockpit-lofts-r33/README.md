# A-12 R33: independent lofts and cockpit study

Explore independent section surfaces, fair guides, and an integrated cockpit in an editable native A-12 appearance study.

![A-12 R33: independent lofts and cockpit study](cover.png)

The R33 graph retains the cockpit study and continuous loft construction. The front-view reference target remains unresolved. It does not establish production dimensions or aerodynamic performance.

## Installation and use

1. Download the package files and keep them together.
2. Open `a12-cockpit-lofts-r33.ntop` in the matching licensed nTop build.
3. Save a working copy before editing. Expand the relevant section and change one control at a time.
4. Inspect the rebuilt bodies and block states before accepting the changed model.

Recorded native build: **6.0.0-rc 42594**. The catalogue version field cannot encode the custom build number. Public release compatibility has not been re-tested. The application and license are not included.

## Inputs and outputs

| Direction | Contents |
|---|---|
| Input | Section guide, surface, and cockpit controls |
| Output | Native fuselage, nacelle, wing and cockpit geometry |

Named scalar controls include `Wingspan`, `Scale factor`, `Body width multiplier`, `Body height multiplier`, `Chine width multiplier`, `Conic section rho`, `Lower body width multiplier`, `Wing thickness multiplier`, `Edge thickness to span`, `Nacelle radius multiplier`, `Nacelle half spacing to span`, `Inlet wall to span`. Some are computed values; inspect their upstream construction before changing them.

## Files

- [a12-cockpit-lofts-r33.ntop](a12-cockpit-lofts-r33.ntop)

## Evidence and source

Publication checks are offline: native-container round trips, export-path changes, collapsed presentation, dependency inspection, and binary payload preservation. These checks do not constitute new native execution. See [publication-checks.json](publication-checks.json).

The [source, usage notes, and recorded report](https://github.com/bradrothenberg/ntop-api-share/tree/92f13bc0a873aacba62526ee43e9e6895ef051f8/demos/a12) retain the original construction and verification scope. Download the public source checkout to view reports with their local assets.

## License

MIT. See [LICENSE](LICENSE). This license covers the original example models, saved recipes, and supporting material in this package.
